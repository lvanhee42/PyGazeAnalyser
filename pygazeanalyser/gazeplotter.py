...
def draw_fixations(fixations, dispsize, imagefile=None, durationsize=True,
                   durationcolour=True, alpha=0.5, savefilename=None):
    ...
    # draw circles
    ax.scatter(fix['x'], fix['y'], s=siz, c=col, marker='o', cmap='jet',
...
def make_heatmap(fix, dispsize, image_data):
    """makes a heatmap of the provided fixations, optionally drawn over an
    image.returns a 2D array with the heatmap values to be saved in memory

    arguments

    fix     		-	a dictionary containing lists of fixation
    ...
    image_data :    -   a object containing a dictionary field 'gaussians' with :
                        -'zoom_i': the gaussian at zoom i ( 4 < i < 10)
    returns

    heatmap			-	a np 2D array containing heatmap values
    """
    ...
    for i in range(0, len(fix['dur'])):
        ...
        if fix['zoom'][i] > 3:
            gaus, zoom_x, zoom_y = image_data.gaussians[
                'zoom_' + str(fix['zoom'][i])]
            x = fix['x'][i] + strt - numpy.int(zoom_x / 2)
            y = fix['y'][i] + strt - numpy.int(zoom_y / 2)
...
def save_heatmap(heatmap, dispsize, imagefile, savefilename, alpha=0.5,
    ...
    heatmap     	-   2D numpy array containing heatmap values.
    ...
    savefilename	-	full path to the file in which the heatmap should be
    				saved, or None to not save the file (default = None)
    avg             - the mean of the heatmap values if already calculated
...
def draw_heatmap(fixations, dispsize, imagefile=None, durationweight=True,
...
def draw_raw(data, dispsize, imagefile=None, savefilename=None):
    ...
    data			-	a dictionary containing 'x' and 'y' tuples as a list of
                    coordinates
    dispsize		-	tuple or list indicating the size of the display,
    				e.g. (1024,768)
    ...
    ax.plot(data['x'], data['y'], 'o', color=COLS['aluminium'][0],
...
def draw_scanpath(fix, dispsize, imagefile, savefilename, alpha=0.5):
    """Draws a scanpath: a series of arrows between numbered fixations,
    	optionally drawn over an image. The fixations are assumed to be
    ...
    ax.scatter(fix['x'], fix['y'], s=fix['dur'], c=COLS['green'][2],
               marker='o', cmap='jet', alpha=alpha,
               edgecolors='none')
    # draw points and order
    for i in range(len(fix['x'])):
        ax.annotate(str(i + 1), (fix['x'][i], fix['y'][i]),
                    color=COLS['aluminium'][5], alpha=1,
                    horizontalalignment='center', verticalalignment='center',
                    multialignment='center')

    # draw arrows
    for i in range(len(fix['x']) - 1):
        ax.arrow(fix['x'][i], fix['y'][i], fix['x'][i + 1] - fix['x'][i],
                 fix['y'][i + 1] - fix['y'][i], alpha=alpha,
                 fc=COLS['aluminium'][0], ec=COLS['aluminium'][5], fill=True,
